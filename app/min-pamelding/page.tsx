...
import { usePamelding } from "@/components/PameldingProvider";
...
export default function MinPamelding() {
  const { valgte, fjern } = usePamelding();
  const [navn, setNavn] = useState("");
  const [skole, setSkole] = useState("Hamar katedralskole");
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800/60">
        <h2 className="mb-4 text-base font-semibold text-slate-900 dark:text-slate-100">Dine opplysninger</h2>
        <div className="flex flex-col gap-4">
          <div>
            <label htmlFor="navn" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">
              Navn
            </label>
            <input
              id="navn"
              ...
              className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 transition-colors focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-400/20 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500 dark:focus:border-indigo-500"
            />
          </div>
          <div>
            <label htmlFor="skole" className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">
              Skole
            </label>
            <input
              id="skole"
              ...
              placeholder="Hamar katedralskole"
              className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 transition-colors focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-400/20 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500 dark:focus:border-indigo-500"
            />
          </div>
        </div>
        ...
        <button
          disabled={valgte.length < 3 || navn.trim() === "" || skole.trim() === ""}
          onClick={() => setBekreftet(true)}
          className="mt-6 w-full rounded-lg bg-indigo-600 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-indigo-500"
        >
          Velg
        </button>
